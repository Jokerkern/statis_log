...
class WebhookNotifier(BaseNotifier):
            ...
            for attempt in range(self.retry_count):
                try:
                    ...
                    if response.status_code >= 200 and response.status_code < 300:
                        logger.info(f"Webhook通知发送成功: {title} (状态码: {response.status_code})")
                        return True
                    else:
                        logger.warning(f"Webhook通知返回非成功状态码: {response.status_code}, 响应: {response.text}")
    ...
    def _prepare_payload(self, title: str, message: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        准备Webhook请求的数据负载
        ...
        if payload_template:
            try:
                ...
                formatted_payload = json.loads(self.format_message(json.dumps(payload_template), payload))
